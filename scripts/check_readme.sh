...
grep -oE '(examples|sidecar|scripts|src|media|manifests|box|deploy)/[A-Za-z0-9_./-]+\.(lex|py|sh|gif|json|md|txt)' README.md \
...
miss=0
for p in $(grep -oE '(examples|sidecar|scripts|src|media|manifests|box|deploy)/[A-Za-z0-9_./-]+\.(lex|py|sh|gif|json|md|txt)' README.md | sort -u); do
  [ -e "$p" ] || miss=1
done
