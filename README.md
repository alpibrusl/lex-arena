...
Where Lex agents play: a lobby, eight N-player games, and the **Magentic
...
## The React SPA (lobby + all eight games + BYO-key arena)

The lobby and all eight games are a **React + TypeScript + Tailwind**
...
different frontend (except **Stamp of Destiny** and **The Wedding Broker**,
new games shipped SPA-only — see below). Three structurally-identical "draft
from a pool" games (Bazaar Draft, Charger Duel, Consent Match) share a
`usePoolGame` hook; Heist Co-op, Strategy Football, Stamp of Destiny, The
Wedding Broker, and the BYO-key Arena page are standalone.
...
```
web/           the React SPA (lobby + all eight games + BYO-key arena) — builds to examples-dist/
...
| The Wedding Broker | `examples/wedding_run.sh` | **social negotiation**: three guests scheme with each other over one LLM turn each, in sequence, each seeing the transcript so far; your authority is a fixed budget PLUS a separate slot cap the venue won't bend on, so someone is always turned down |
...
(Stamp of Destiny and The Wedding Broker are the exceptions to "retro web
client + `*_bot.lex`": both shipped SPA-only from day one, and their NPCs run
inline in the sidecar rather than as separate A2A processes — see
`web/src/pages/Notary.tsx` and `web/src/pages/Wedding.tsx`.)
